    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--bd)', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={onLogin} style={{ ...btn2, padding: '7px 14px', fontSize: '13px' }}>Inloggen</button>
          <button onClick={onRegister} style={{ ...btn1, padding: '7px 14px', fontSize: '13px' }}>Registreren</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ maxWidth: '640px', textAlign: 'center' }}>
          <h1 style={{ fontSize: '30px', fontWeight: 800, letterSpacing: '-.02em', lineHeight: 1.25, margin: '0 0 14px' }}>
            Compliant facturen voor de bouw,<br />in drie stappen
          </h1>
          <p style={{ fontSize: '15px', color: 'var(--tm)', lineHeight: 1.7, margin: '0 auto 24px', maxWidth: '520px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '14px', flexWrap: 'wrap' }}>
            ...
            <button onClick={onLogin} style={{ ...btn2, padding: '11px 26px', fontSize: '14px' }}>
              Inloggen
            </button>
          </div>
          ...
          <a
            href="#/uitleg"
            style={{ fontSize: '13px', color: 'var(--ac)', textDecoration: 'underline' }}
          >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '12px', marginTop: '40px', textAlign: 'left' }}>
            {[
              ...
              <div key={f.t} style={{ border: '1px solid var(--bd)', borderRadius: '8px', padding: '14px' }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--tm)', lineHeight: 1.6 }}>{f.d}</div>
              </div>
            ))}
          </div>
        </div>
